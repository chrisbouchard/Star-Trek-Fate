...
\begin{SkillActionList}
    ...
    \aspect{Oxygenated Blood} if a character will have to work in a low-oxygen
    environment.}
...
\emph{(Note: There is no actual system in place for modifying Medicine actions
based on the available sickbay. It is currently left up to GM discression.)}
...
aspect it has a class aspect, such as \aspect{Constitution-Class}.
...
gaining the aspect \aspect{Eerily Dark} when the power goes out.
...
If the related skill is \AdjLevel{1} or \AdjLevel{2}, add one box (for a total
of three). If the skill is \AdjLevel{3} or higher, add two boxes (for a total
of four). If the skill is \AdjLevel{5}, the ship also gets an extra mild
consequence.
